...
export function buildStickerPrompt(values: DialValues) {
  ...
  return [
    ...
    `Stamp construction: ${values.stamp.orientation}, ${values.stamp.imageArea}% illustration area, ${values.stamp.borderWidth}px cream paper border, ${values.stamp.perforation}px rounded perforations, ${values.stamp.copyLayout} copy layout.`,
    ...
    "The output must look like a complete collectible stamp, not a mockup, UI screenshot, loose illustration, or card on a desk. Keep all lettering legible and spelled correctly. Use a 3:4 portrait aspect ratio.",
  ].join("\n");
}
